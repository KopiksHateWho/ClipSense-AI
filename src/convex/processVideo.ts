...
async function transcribeWithGroq(audioBase64: string, apiKey: string): Promise<GroqSegment[]> {
  ...
  return (data.segments || []).map((s: GroqSegment) => ({ text: s.text, start: s.start, end: s.end }));
}

async function transcribeWithDeepgram(audioBase64: string, apiKey: string): Promise<GroqSegment[]> {
  const audioBytes = Uint8Array.from(atob(audioBase64), (c) => c.charCodeAt(0));
  const res = await fetch("https://api.deepgram.com/v1/listen?model=nova-2&smart_format=true&paragraphs=true", { method: "POST", headers: { Authorization: `Token ${apiKey}`, "Content-Type": "audio/webm" }, body: audioBytes });
...
async function transcribeWithOpenAI(audioBase64: string, apiKey: string): Promise<GroqSegment[]> {
  ...
  const data = await res.json();
  return (data.segments || []).map((s: WhisperSegment) => ({ text: s.text, start: s.start, end: s.end }));
}
...
async function scoreWithClaude(transcript: string, audioHints?: string): Promise<ClipResult[]> {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2048, messages: [{ role: "user", content: buildScoringPrompt(transcript, audioHints) }] }),
  });
...
async function scoreWithOpenAI(transcript: string, audioHints?: string): Promise<ClipResult[]> {
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "gpt-4o-mini", messages: [{ role: "user", content: buildScoringPrompt(transcript, audioHints) }], response_format: { type: "json_object" } }),
  });
  if (!res.ok) throw new Error(`OpenAI: ${await res.text()}`);
  const data = await res.json();
...
function generateMockData(videoId: string): { transcript: GroqSegment[]; clips: ClipResult[] } {
  ...
  while (indices.size < 5 && indices.size < transcript.length) indices.add((seed + indices.size * 7) % transcript.length);
  let idx = 0;
  for (const i of Array.from(indices).sort((a, b) => a - b)) {
    const s = transcript[i];
    clips.push({ startTime: Math.max(0, s.start - 3), endTime: s.start + 30 + (seed + idx) % 30, score: 0.7 + ((seed + idx * 11) % 30) / 100, label: labels[(seed + idx) % labels.length], reason: "AI-detected highlight moment" });
    idx++;
  }
...
export const processVideo = action({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; clipCount: number; realPipeline: boolean }> => {
    ...
    let hasRealAPIs = false;
    try {
      settings = (await ctx.runQuery(api.apiSettings.get)) as Record<string, unknown> || {};
      hasRealAPIs = !!(settings?.transcriptionApiKey && settings?.llmApiKey);
    } catch (e) { console.log("[Pipeline] Could not fetch API settings, using mock:", e); }
    ...
    try {
      if (hasRealAPIs && args.audioBase64) {
        ...
        if (args.audioEnergyData) {
          try {
            ...
            if (energyData.length > 0) audioHints = energyData.map((e) => `[${Math.floor(e.startTime)}s-${Math.floor(e.endTime)}s] ${e.type}: energy=${(e.score * 100).toFixed(0)}%`).join("\n");
          } catch { /* ignore */ }
        }
        ...
        await ctx.runMutation(api.jobs.updateStatus, { jobId: args.jobId, status: "transcribing", progress: 30 });
        await new Promise((r) => setTimeout(r, 1000));
        const mock = generateMockData(videoId);
        transcript = mock.transcript;
        clips = mock.clips;
        await ctx.runMutation(api.jobs.updateStatus, { jobId: args.jobId, status: "analyzing", progress: 65 });
        await new Promise((r) => setTimeout(r, 1000));
      }
      ...
      await ctx.runMutation(api.jobs.updateStatus, {
        jobId: args.jobId, status: "failed", progress: 0,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      return { success: false, clipCount: 0, realPipeline };
    }
  },
});
